package com.ixiyou.speUI.util 
{
	/**
	 * 此类用于查找显示对象
	 * 参考 flashyiyi 的ghostcat.util.display.SearchUtil
	 * @author spe
	 */
	import flash.display.DisplayObject
	import flash.display.DisplayObjectContainer
	public class SearchDisplayObject
	{
		/**
         ...
         * 
         */        
        public static function findThisChildByProperty(displayObj:DisplayObjectContainer, property:String, value:*=null, depth:int = int.MAX_VALUE):DisplayObject
        {
			   for (var i:int=0;i < displayObj.numChildren;i++) 
                {
                    var child:DisplayObject = displayObj.getChildAt(i)
					if (child.hasOwnProperty(property)) {
						if (value && child[property] == value || value==null)return displayObj;	
					}
                }
				return null
		}
         ...
         * 
         */        
        public static function findThisAllChildByProperty(displayObj:DisplayObjectContainer, property:String, value:*=null, depth:int = int.MAX_VALUE):Array
        {
		   ...
		   for (var i:int=0;i < displayObj.numChildren;i++) 
			{
				var child:DisplayObject = displayObj.getChildAt(i)
				if (child.hasOwnProperty(property)) {
					if (value && child[property] == value || value == null) {
		 ...
		 * @return
		 */
		public static function findThisChildByClass(displayObj:DisplayObjectContainer, classRef:Class, depth:int = int.MAX_VALUE):DisplayObject
        {
                for (var i:int=0;i < displayObj.numChildren;i++) 
                {
                    var child:DisplayObject = displayObj.getChildAt(i)
					if (child is classRef)return child;	
                }
		 ...
		 * @return
		 */
		public static function findThisAllChildByClass(displayObj:DisplayObjectContainer, classRef:Class, depth:int = int.MAX_VALUE):Array {
			var result:Array=null
			 for (var i:int=0;i < displayObj.numChildren;i++) 
			{
				var child:DisplayObject = displayObj.getChildAt(i)
				if (child is classRef) {
					if (result == null) result = new Array()
					result.push(child)
				}
			}
