package com.ixiyou.speUI.controls 
{
	...
	import com.ixiyou.speUI.controls.skins.VSliderSkin
	import flash.geom.Rectangle;
	import flash.text.TextField;
	import caurina.transitions.Tweener
	public class MVSlider extends SpeComponent implements ISlider,ISkinComponent
	{
		//是否背景点击
		...
		public function MVSlider(config:*=null) 
		{
			super(config)
		...
		private function mouseBgDown(e:MouseEvent):void {
			...
			var num:Number = Number(middleNum * (this.mouseY / (this.height - _slider.height)) + minimum)
		...
		private function mouseDown(e:MouseEvent):void {
			_slider.startDrag(false, new Rectangle(0, 0, 0,this.height - _slider.height))
			_stage=this.stage
		...
		private function mouseMove(e:MouseEvent):void {
			if (liveDragging) {
				...
				_sliderMask.width = _sliderState.width
				_sliderMask.height = (height - _slider.height) * ((_value-minimum) / middleNum) + _slider.height / 2;
				...
				_sliderMask.width = _sliderState.width
				_sliderMask.height = (height-_slider.height) * ((num-minimum)/ middleNum)+_slider.height/2;
			}
		}
		/**
		 * 计算遮罩
		...
		private function computeMask():void {
			_sliderMask.width= _sliderState.width
			Tweener.addTween(_sliderMask,{time:.2,height:(height-_slider.height)*computeNumFormValue()+_slider.height/2})
		}
		/**
		 * 计算滑块位置
		 */
		private function computeSliderX():void {
			if (_slider) {
				Tweener.addTween(_slider,{time:.2,y:(height - _slider.height)* computeNumFormValue()})
		...
		private function computeNumFormSlider():Number {
			return	_slider.y/(this.height-_slider.height)
		}
		/**
		 * 根据值来计算值百分比
		 * @return
		...
		public function set skin(value:*):void {
			...
			}else if (value == null){skin=new VSliderSkin()}
		}
		/**
		 * 指定在轨道上单击是否会移动滑块。
		 */
		...
		override public function upSize():void {
			if (_slider) {
				computeMask()
				computeSliderX()
				_sliderBg.height = this.height
				_sliderState.height = this.height
			}
		}
		/**
		 * 重写删除
